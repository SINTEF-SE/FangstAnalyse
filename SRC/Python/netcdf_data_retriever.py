...
class DumpNetCDFData(object):
    ...
    def retrieve_data(self):
        ...
        new_frame = g["x_wind_10m", "y_wind_10m", "air_pressure_at_sea_level", "air_temperature_2m"].sum().reset_index()
        new_frame.to_csv(sys.stdout, index=False)
        #g.x_wind_10m.sum().reset_index()
        #g.y_wind_10m.sum().reset_index()
        #yaface = pd.DataFrame(g.size())
        #header = ["latitude", "longitude", "x_wind_10m", "y_wind_10m"]
      #  df.to_csv("testfile.csv", columns=header, index=False)
